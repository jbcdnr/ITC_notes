...
\begin{proof}
  \begin{align*}
    \Pr{Y^n=y^n, X^n=x^n} &= \prod_{i=1}^n p(x_i|x^{i-1}y^{i-1})W_i(y^i|x^iy^{i-1})\\
...
\begin{lemma}
  if $W$ is stationary memoryless and there si no feedback, then
  \[
...
\begin{theorem}{"Bad news" theorem, converse to the coding theorem}
  \begin{itemize}
    ...
    \item Suppose also that we have a channel $W$ that acceots input $X_1 X_2 ...$ once every $\tau_c$ seconds.
    \item Suppose also 
    \begin{align*}
...
We could show that if $\frac{H}{\tau_s} \leq \frac{c(w)}{\tau_c}$ then for any $\lambda > 0$, we can find a stable encoder and decoder such taht $p_e < \lambda$. 
